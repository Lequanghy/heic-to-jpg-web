...
async def home():
    ...
    <body class="bg-gradient-to-br from-purple-50 to-pink-50 min-h-screen flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl w-full">
        ...
        <p class="text-center text-gray-600 mb-8">Free, instant, no upload limits</p>

        <form id="uploadForm" class="space-y-6">
          <div id="dropZone" class="border-4 border-dashed border-purple-300 rounded-xl p-12 text-center cursor-pointer hover:border-purple-500 transition">
            <i class="fas fa-cloud-upload-alt text-6xl text-purple-400 mb-4"></i>
            <p class="text-xl text-gray-700">Drop HEIC files here or click to browse</p>
            <input type="file" name="files" id="fileInput" multiple accept=".heic,.HEIC,.heif,.HEIF" class="hidden" />
          </div>

          <div id="progress" class="hidden">
            <div class="text-sm text-gray-600 mb-2">Converting...</div>
            <div class="w-full bg-gray-200 rounded-full h-3">
              <div id="progressBar" class="bg-gradient-to-r from-purple-500 to-pink-500 h-3 rounded-full transition-all" style="width: 0%"></div>
            </div>
          </div>
      ...
      <script>
        ...
        ['dragover', 'dragenter'].forEach(e => {
          dropZone.addEventListener(e, () => dropZone.classList.add('border-purple-600', 'bg-purple-50'));
        });
        ['dragleave', 'dragend', 'drop'].forEach(e => {
          dropZone.addEventListener(e, () => dropZone.classList.remove('border-purple-600', 'bg-purple-50'));
        });
        ...
        async function handleFiles(files) {
          ...
          for (const file of heicFiles) {
            ...
            try {
              ...
              div.innerHTML = `
                <span class="font-medium text-green-800">${file.name} → ${file.name.replace(/\.hei[c|f]/i, '.jpg')}</span>
                <a href="${url}" download="${file.name.replace(/\.hei[c|f]/i, '.jpg')}"
              ...
              completed++;
              progressBar.style.width = (completed / heicFiles.length * 100) + '%';
            } catch (err) {
